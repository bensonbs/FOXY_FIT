...
def train(hyp, opt, device, callbacks):  # hyp is path/to/hyp.yaml or hyp dictionary
    ...
    for epoch in stqdm(range(start_epoch, epochs)):  # epoch ------------------------------------------------------------------
        v2.markdown(f'## Training epoch {epoch+1}')
        ...
        optimizer.zero_grad()
        for i, (imgs, targets, paths, _) in stqdm(pbar):  # batch -------------------------------------------------------------
...
def parse_opt(known=False):
    ...
    parser.add_argument('--data', type=str, default=ROOT / f'{username}/datasets/coco.yaml', help='dataset.yaml path')
    ...
    parser.add_argument('--project', default=ROOT / f'{username}/runs/train', help='save to project/name')
...
def make_training_data(username):               
    ...
    for project in projects:
        ...
        for train_file in train_files:
            pic = os.path.join(*([username] + ['temp'] + [project] + train_file.split('/')[1:]))
            pic_name = os.path.basename(pic)

            # pic
            output_path = os.path.join(username,'datasets','train',project,'images')
            ...
            output_path = os.path.join(username,'datasets','train',project,'labels')
        ...
        for valid_file in valid_files:
            pic = os.path.join(*([username] + ['temp'] + [project] + valid_file.split('/')[1:]))
            pic_name = os.path.basename(pic)

            # pic
            output_path = os.path.join(username,'datasets','valid',project,'images')
            check_path(output_path)
            output_pic = os.path.join(output_path,pic_name)
            if not os.path.isfile(output_pic):
                os.rename(pic,output_pic)
            ...
            ann = pic.replace('png','txt')
            ann_name = os.path.basename(ann)
            output_path = os.path.join(username,'datasets','valid',project,'labels')
            if not os.path.isdir(output_path):
                os.makedirs(output_path)
            output_ann = os.path.join(output_path,ann_name)
            if not os.path.isfile(output_ann):
    ...
    with open(os.path.join(username,'temp',project,'obj.names'),'r') as f:
        names = f.readlines()

    names = [''.join(name.replace('\n','')) for name in names]

    d = {
        'path' : os.path.join(os.getcwd(),username,'datasets'), 
    ...
    with open(os.path.join(username,'datasets','coco.yaml'), 'w') as f:
        yaml.dump(d, f, sort_keys=False)

def zip_files(username):
    with zipfile.ZipFile(f'{username}/output.zip', 'w', zipfile.ZIP_DEFLATED) as zipf:
        zipdir(f'{username}/runs/train/exp', zipf)
...
if __name__ == "__main__":
    ...
    with st.sidebar:
        ...
        st.image('pic/4.png')
        #Title
        # new_title = '<p style="font-family:sans-serif; color:Orange; font-size: 42px;"> AI Training </p>'
        # st.markdown(new_title, unsafe_allow_html=True)

        #Icon
        # with open('./lottie/ModelBall.json','r') as f:
    ...
    if not authentication_status:
        ...
        st.markdown(f'### `{usr[0]}` 正在訓練，請稍後重新整理頁面')
            ...
            if len(uploaded_files) == 0:
                ...
                try:
                    ...
                    ```
                        ''')
                    block = True
        if len(uploaded_files) > 0 and not block:
            if col[0].button('Train',disabled = block):
                ...
                with L1:
                    ...
                    with col[1]:
                            v1 = st.empty()
                            v2 = st.empty()
                            v1.warning('已開始訓練流程，請勿關閉此分頁', icon="⚠️")
                            v2.info('資料處理中 請稍後..', icon="🚀")
